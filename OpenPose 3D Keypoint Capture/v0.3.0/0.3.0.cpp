...
std::array<int, 3> CVKeypoints = {0, 16, 17};
int keypointNum = CVKeypoints.size();		//note: keypointNum must be manually entered into the MATLAB array definitions
...
cv::Mat camera1CV(keypointNum, 2, CV_64F);
cv::Mat camera2CV(keypointNum, 2, CV_64F);


//UndistortPoints Matrices
cv::Mat camera1Distorted(keypointNum, 1, CV_64FC2);
cv::Mat camera2Distorted(keypointNum, 1, CV_64FC2);
cv::Mat camera1Undistorted(keypointNum, 1, CV_64FC2);
cv::Mat camera2Undistorted(keypointNum, 1, CV_64FC2);
cv::Mat camera1Unmatched(1, keypointNum, CV_64FC2);
cv::Mat camera2Unmatched(1, keypointNum, CV_64FC2);
cv::Mat camera1Matched(1, keypointNum, CV_64FC2);
cv::Mat camera2Matched(1, keypointNum, CV_64FC2);
cv::Mat camera1toTriangulate(keypointNum, 1, CV_64FC2);
cv::Mat camera2toTriangulate(keypointNum, 1, CV_64FC2);

//4D keypoint matrix
cv::Mat points4D(keypointNum, 4, CV_64F);
...
void outputBodyKeypoints(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr, op::Array<float> arr, int whichCamera)
{
	try
	{
		if (datumsPtr != nullptr && !datumsPtr->empty())
		{
			...
			{
				int j = CVKeypoints[i];							//get selected keypoints
...
void triangulatePoints()
{
	...
	for (int i = 0; i < keypointNum; i++)
	{
		...
		op::opLog("X: " + std::to_string(x), op::Priority::High);
		op::opLog("Y: " + std::to_string(y), op::Priority::High);
		op::opLog("Z: " + std::to_string(z), op::Priority::High);
	}
}
