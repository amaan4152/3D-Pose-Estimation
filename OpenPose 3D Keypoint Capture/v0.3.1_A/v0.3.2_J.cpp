...
std::vector<int> CVBodyKeypoints{0, 1, 2, 3, 4, 5, 6, 7, 8, 15, 16, 17, 18};
...
double confidenceBound = 0.15;

//Definition of checkerboard used in cameraCalibration
int boardWidth = 9;
int boardHeight = 7;
float squareSize = 0.7874;			//inches
...
std::vector<double> spf_vals, mov_avgs; 

double x, y, z, w;

// This worker will just read and return all the jpg files in a directory
bool display(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr, bool dualDis)
{
	try
	{
...
int cameraCalibration()
{
	...
	//for each frame
	for (i = 0; numSuccess < numFrames; i++)
	{
		//capture frames
		camera1 >> frame1;
		camera2 >> frame2;
...
void triangulatePoints(int & frameNum, int & i_tick)
{
	std::ofstream outfile("out_new.json");
	op::JsonOfstream json_out("out_new.json"); //create OP json parser object
	json_out.objectOpen();
	if (body)
	{
		//get keypoints
		...
		/*
		int f_tick = cv::getTickCount();
		double spf = (f_tick - i_tick)/cv::getTickFrequency();
		i_tick = f_tick;

		double n = 0;
		if(spf_vals.size() == 30)
		{
			spf_vals.insert(spf_vals.begin(), spf);
			spf_vals.pop_back();
		}
		if(spf_vals.size() < 30)
		{
			spf_vals.push_back(spf);
		}
		else
		{
			
			for(double i: spf_vals)
				n+=i;
			n /= 30; //moving average			 
		}
		*/
		...
		json_out.key("frame_num");
		json_out.plainText(frameNum);
		json_out.comma();
		json_out.enter();
		for (int i = 0; i < bodyKeypointNum; i++)
		{
			...
			json_out.key(std::to_string(j)); //keypoint #i array object of coordinates
			json_out.objectOpen();
			json_out.key("xyz");
			json_out.enter();
			json_out.arrayOpen();
			json_out.plainText(x); //FOR BLENDER 
			json_out.comma();
			json_out.plainText(-y); //FOR BLENDER
			json_out.comma();
			json_out.plainText(z); //FOR BLENDER
			json_out.arrayClose();
			json_out.objectClose();
			if(i < bodyKeypointNum - 1)
				json_out.comma();
			
		}
	}
	json_out.objectClose(); //complete JSON file for body keypoints
...
int cameraCapture3D()
{
	try
	{
		...
		MultiCam_TH multiCams = MultiCam_TH(cams);
		//open video capture
		camera1.open(cams.at(0), cv::CAP_ANY);
		camera2.open(cams.at(1), cv::CAP_ANY);
		//err detection
		if (!camera1.isOpened())
		{
		...
		while (true)
		{
			...
			//save frame as jpeg
			frame1 = multiCams.inputData[0]; //get frame from cam 0 (thread 0)
			frame2 = multiCams.inputData[1]; //get frame from cam 1 (thread 1)
			cv::imwrite(filepath1, frame1);
			cv::imwrite(filepath2, frame2);
			// Process and display images
			int tick_0 = cv::getTickCount();
			for (const auto& imagePath : imagePaths)	//for loop processses one frame from each camera before looping
			{
				...
				if (datumProcessed != nullptr)
				{
					...
					{
						const auto userWantsToExit = display(datumProcessed, dual);
						dual = !dual;
						if (userWantsToExit)
						{
